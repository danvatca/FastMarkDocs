...
class TestOpenAPIEnhancer:
    ...
    def test_merge_response_examples_with_existing(self):
        ...
        response_examples = [
            ResponseExample(
                status_code=200,
                description='New response example',
                content=[{'id': 1, 'name': 'John'}]
            )
        ]
    ...
    def test_custom_headers_in_code_samples(self, sample_openapi_schema):
        ...
        if 'x-codeSamples' in get_operation:
            curl_sample = next(
                (s for s in get_operation['x-codeSamples'] if s['lang'] == 'curl'),
                None
            )
            ...
            endpoints.append(
                EndpointDocumentation(
                    ...
                    code_samples=[
                        CodeSample(
                            language=CodeLanguage.CURL,
                            code=f'curl -X GET "https://api.example.com/api/endpoint{i}"'
                        )
                    ]
                )
            )
...
class TestEnhanceOpenAPIWithDocs:
    ...
    def test_enhance_openapi_with_docs_basic(self, sample_openapi_schema, temp_docs_dir, sample_markdown_content, test_utils):
    ...
    def test_enhance_openapi_with_docs_custom_config(self, sample_openapi_schema, temp_docs_dir, sample_markdown_content, test_utils):
