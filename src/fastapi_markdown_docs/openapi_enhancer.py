...
from .exceptions import OpenAPIEnhancementError, DocumentationLoadError
...
def enhance_openapi_with_docs(
    ...
    try:
        ...
        raise OpenAPIEnhancementError(
            "enhancement_failed",
            f"Failed to enhance OpenAPI schema: {str(e)}"
        )
...
class OpenAPIEnhancer:
    ...
    def __init__(
        ...
        self.code_generator = CodeSampleGenerator(
            ...
            code_sample_languages=self.code_sample_languages
        )
    
    def enhance_openapi_schema(self, openapi_schema: Dict[str, Any], documentation: DocumentationData) -> Dict[str, Any]:
        """
        ...
        if openapi_schema is None:
            raise OpenAPIEnhancementError(
                "schema_validation",
                "OpenAPI schema cannot be None"
            )
        
        if documentation is None:
            raise OpenAPIEnhancementError(
                "documentation_validation", 
                "Documentation data cannot be None"
            )
        ...
        if not isinstance(openapi_schema, dict):
            raise OpenAPIEnhancementError(
                "schema_validation",
                "OpenAPI schema must be a dictionary"
            )
        ...
        if 'openapi' not in openapi_schema and 'swagger' not in openapi_schema:
            raise OpenAPIEnhancementError(
                "schema_validation",
                "Invalid OpenAPI schema: missing 'openapi' or 'swagger' field"
            )
        
        if 'info' not in openapi_schema:
            raise OpenAPIEnhancementError(
                "schema_validation",
                "Invalid OpenAPI schema: missing 'info' field"
            )
        ...
        try:
            ...
            stats = {
                'endpoints_enhanced': 0,
                'code_samples_added': 0,
            ...
            raise OpenAPIEnhancementError(
                "schema_root",
                f"Schema enhancement failed: {str(e)}"
            )
    ...
    def _enhance_operation(self, operation: Dict[str, Any], endpoint_doc: EndpointDocumentation, stats: Dict[str, int]):
        ...
        if self.include_code_samples:
            ...
            filtered_generated = [
                sample for sample in generated_samples 
                if sample.language in self.code_sample_languages
            ]
            ...
            if endpoint_doc.code_samples:
                ...
                for existing_sample in endpoint_doc.code_samples:
                    if (existing_sample.language not in existing_languages and 
                        existing_sample.language in self.code_sample_languages):
    ...
    def _add_code_samples_to_operation(self, operation: Dict[str, Any], code_samples: List[CodeSample], stats: Dict[str, int] = None):
        """
    ...
    def _add_response_examples_to_operation(self, operation: Dict[str, Any], response_examples: List[ResponseExample], stats: Dict[str, int] = None):
        """
    ...
    def _add_parameter_examples(self, operation: Dict[str, Any], parameters: List[ParameterDocumentation], stats: Dict[str, int]):
        """
    ...
    def _paths_match(self, openapi_path: str, doc_path: str) -> bool:
        ...
        for openapi_part, doc_part in zip(openapi_parts, doc_parts):
            ...
            if openapi_part.startswith('{') and openapi_part.endswith('}') and \
               doc_part.startswith('{') and doc_part.endswith('}'):
                continue
